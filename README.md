...
5. 使用“今日验收”筛选查看今天必须交付或确认结果的任务；它会显示今天的高优先级任务，以及带有验收、交付、检查、检测、测试、UAT、CCD、今日计划或效果图等信号的任务。
6. 使用“日历规划”查看指定日期安排，使用“数据管理”导出或导入 JSON 备份。
7. 在“设置”中查找或修改编辑模式快捷键、透明度、置顶、开机自启和存储方式。旧版 `~+1` 仍可兼容，但在游戏、管理员窗口或反作弊环境中可能失效，建议使用标准组合键。
8. CLI、AI 或其他本地程序提交的任务会进入“外部提案”，确认后才加入正式任务列表。
9. 在“日程规划”中选择今天或明天、任务列表模式或时间块模式，使用本地算法生成计划建议。
10. 在“目标库”中维护长期目标、优先级、每日投入和最近里程碑，日程规划会自动读取进行中的目标。
...
TaskOverlay.Cli.exe task list --filter todayAcceptance
...
如果主程序以管理员身份启动后已经能在游戏内识别快捷键，就不要再启动桥接程序监听同一个快捷键。主程序和桥接程序同时收到同一组合键时，会连续触发两次切换，表现为“按了但没有变化”。需要桥接时，推荐二选一：

- 主程序普通权限运行，桥接程序管理员权限运行，使用同一个快捷键。
- 主程序管理员权限运行，不启动桥接程序。

如果必须同时运行两者，请给桥接程序指定一个和主程序不同的快捷键，例如 `--hotkey "Ctrl+Shift+F12"`。
